...
void send_init_poses(void) {
  	...
  	// Get data
            loc[1][0] = wb_supervisor_field_get_sf_vec3f(robs_trans[i])[0]; // X
            loc[1][1] = wb_supervisor_field_get_sf_vec3f(robs_trans[i])[2]; // Z
            loc[1][2] = wb_supervisor_field_get_sf_rotation(robs_rotation[i])[3]; // THETA
            
            // Send it out
            sprintf(buffer,"%1d#%f#%f#%f##%f#%f",i+offset,loc[1][0],loc[1][1],loc[1][2], migrx, migrz);
