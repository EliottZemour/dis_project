...
//#define DATASIZE 2*NB_SENSOR + 2         // Number of elements in particle (2 Neurons with 8 proximity sensors 
                                        // + 2 bias)
...
void reset(void) {
  ...
  for (i = 0; i < ROBOTS; i++)
  {
    ...
    sprintf(robot_name, "epuck_%d_%d", i / FLOCK_SIZE, i % FLOCK_SIZE); // not sure abt that
...
void calc_fitness(double weights[ROBOTS][DATASIZE], double fit[ROBOTS], int its, int numRobs) {
  ...
  int i,j;
    ...
    printf("PSO_SUP: went into calc_fitness\n");
  ...
  for (i=0;i<numRobs;i++) {
    ...
    buffer[DATASIZE] = its;
    printf("PSO_SUP_208: emmiter function \n");
    
    int ppt = wb_emitter_send(emitter[i],(void *)buffer,(DATASIZE+1)*sizeof(double));
    printf("return value %d\n",ppt);//This function returns 1 if the message was placed in the sending queue, 0 if the sending queue was full
  }

  /* Wait for response */
  printf("217_SUP sent data to robot");
